...
const getTags = async () => {
  const tags = await Tag.find({}, { name: 1, interviews: 1 });
  return tags;
};
...
const calcDiff = (allFavItv, receivedSet) => {
  const difference = new Set([...allFavItv].filter(v => !receivedSet.has(v)));
...
const sendAllMail = async emailAndItvArr => {
  await Promise.all(emailAndItvArr.map(sendEachMail));
};
...
const updateAllreceived = async emailAndItvArr => {
  await Promise.all(emailAndItvArr.map(updateEachRecived));
};

export const handler = async () => {
  db.connect();
  const subscribers = await getSubs();
  const emailAndItvArr = await Promise.all(subscribers.map(choiceInterview));
  await Promise.all([
